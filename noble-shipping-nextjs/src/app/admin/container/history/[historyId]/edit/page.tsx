...
export default function EditContainerHistory({ params }: { params: Promise<{ historyId: string }> }) {
  ...
  useEffect(() => {
    params.then(({ historyId }) => {
      ...
      fetch(`/api/admin/container/history/${historyId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const onSubmit = async (data: HistoryFormData) => {
    ...
    try {
      const response = await fetch(`/api/admin/container/history/${historyId}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        router.push(`/admin/container/${cargoId}/history`)
      } else {
