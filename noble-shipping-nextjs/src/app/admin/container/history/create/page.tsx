...
export default function CreateContainerHistory() {
  ...
  const { register, handleSubmit } = useForm<HistoryFormData>({
    defaultValues: {
      cargo_id: cargoId ? parseInt(cargoId) : 0,
  ...
  const onSubmit = async (data: HistoryFormData) => {
    ...
    try {
      const response = await fetch('/api/admin/container/history', {
        method: 'POST',
      ...
      <Card className="shadow-lg border-0">
        <CardContent className="p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <input type="hidden" {...register('cargo_id')} />
